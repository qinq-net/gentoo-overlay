...
src_prepare() {
	...
	LANG=C LC_ALL=C find core/zip -type f -name "[a-z]*" -print0 | xargs -0 rm
...
src_install() {
	...
	if use pythia8; then
		echo "PYTHIA8=${EPREFIX}/usr" >> 99root-${MY_PV} || die
	fi

	if use python; then
		echo "PYTHONPATH=${EPREFIX}/${MY_PREFIX}/lib" >> 99root-${MY_PV}
